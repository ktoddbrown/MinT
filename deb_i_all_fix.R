deb_i_all_fix<-function(data, FACT){
  ...
  if(FACT==1){
    ...
    parnames<-c("Vmax", "Km", "m0", "f", "Yu", "fpr", "fps", "fds")
    ...
    estim<-function(odeset){
      ...
      cost<-function(x){
        ...
        yhat_all<-as.data.frame(ode(y=c(R=0.02988069, S=0.4958934, C=25), parms=par, deriv, times=sort(odeset$Time)))
      ...
      par_mcmc<-modMCMC(f=cost, p=c(Vmax=0.1, Km=3, m0=0.01, f=4, Yu=0.8, fpr=0.5, fps=0.5, fds=0.03),
                          lower=c(Vmax=1e-5, Km=1e-2, m0=1e-5, f=1e-3, Yu=0.01, fpr=0, fps=0, fds=0),
                          upper=c(Vmax=1, Km=20, m0=1, f=100, Yu=0.8, fpr=1, fps=1, fds=1), niter=10000)
