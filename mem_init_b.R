mem_init_b<-function(data, SUB, FACT, Mean, Niter, DNAci){
  ...
  if(SUB==TRUE){
    ...
    for(i in unique(dat$id)){
      obs_Protc<-append(obs_Protc, cost_Protc(res[[i]]$bestpar, data=dat[dat$id==i, ])$residuals$obs)
      mod_Protc<-append(mod_Protc, cost_Protc(res[[i]]$bestpar, data=dat[dat$id==i, ])$residuals$mod)
    }
