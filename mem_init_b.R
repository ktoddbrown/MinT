mem_init_b<-function(data, SUB, FACT, Mean, Niter, DNAci){
  ...
  if(SUB==TRUE){
    ...
    mem<-function(X, pars, t){
      ...
      deriv<-function(time, state, pars){
        ...
        with(as.list(c(state, pars)),{
          ...
          #DNA and Protein calculation
          #DNAc=fd*Cmic
          Protc=fp*Cmic
          
          return(list(c(dCmic, dC, dE),  Protc=Protc, r=(1-CUE)*Vmax*Cmic*C/(Km+C)))
    ...
    estim<-function(data){
      ...
      ###Glucose
      gl<-select(filter(data, Substrate=="Glucose" ), c("Time", "r", "E" , "Protc"))
      colnames(gl)<-c("time", "r", "E" ,"Protc")
      ...
      ###Cellobiose
      cel<-select(filter(data, Substrate=="Cellobiose" ), c("Time", "r", "E" ,"Protc"))
      colnames(cel)<-c("time", "r1", "E1" ,"Protc1")
      ...
      ###Mix
      mix<-select(filter(data, Substrate=="Mix" ), c("Time", "r", "E" , "Protc"))
      colnames(mix)<-c("time", "r2", "E2" ,"Protc2")
      ...
      res<-modMCMC(f=cost_function, p=c(Vmax=0.1, Km=3, CUE=0.8, kmic=0.01, ke=0.01, pe=0.01, fp=0.5, Cmic_0=0.12),
                   lower=c(Vmax=1e-4, Km=1e-4, CUE=1e-2, kmic=1e-5, ke=1e-6, pe=1e-6,  fp=0, Cmic_0=1e-6),
                   upper=c(Vmax=1e4, Km=1e4, CUE=0.999, kmic=1e5, ke=1e6, pe=1e6, fp=1, Cmic_0=25),niter=Niter)
    ...
    # 
    # for(i in unique(dat$id)){
    #   obs_DNAc<-append(obs_DNAc, cost_DNAc(summary(res[[i]])[1,], data=dat[dat$id==i, ])$residuals$obs)
    #   mod_DNAc<-append(mod_DNAc, cost_DNAc(summary(res[[i]])[1,], data=dat[dat$id==i, ])$residuals$mod)
    # }
    # 
    ...
    for(i in unique(dat$id)){
      obs_Protc<-append(obs_Protc, cost_Protc(summary(res[[i]])[1,], data=dat[dat$id==i, ])$residuals$obs)
      mod_Protc<-append(mod_Protc, cost_Protc(summary(res[[i]])[1,], data=dat[dat$id==i, ])$residuals$mod)
    }
    ...
    al$OvP_r<-OvP_r
    al$ll<-rbind(likelihood_r, likelihood_Protc, likelihood_E)
    ...
    estim<-function(data){
      ...
      Obs_dat<-select(data, c("Time", "r","E", "Protc"))
      colnames(Obs_dat)<-c("time", "r","E", "Protc")
