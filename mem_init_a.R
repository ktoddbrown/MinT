mem_init_a<-function(data, SUB, FACT, Mean, Niter, DNAci){
  ...
  if(SUB==TRUE){
    ...
    mem<-function(X, pars, t){
      ...
      deriv<-function(time, state, pars){
        ...
        with(as.list(c(state, pars)),{
          ...
          return(list(c(dCmic, dC, dE),  DNAc=DNAc, r=(1-CUE)*Vmax*Cmic*C/(Km+C)))
    ...
    estim<-function(data){
      ...
      res<-modMCMC(f=cost_function, p=c(Vmax=0.1, Km=3, CUE=0.8, kmic=0.01, ke=0.01, pe=0.01, fd=0.05, Cmic_0=0.12),
                   lower=c(Vmax=1e-4, Km=1e-4, CUE=1e-2, kmic=1e-5, ke=1e-6, pe=1e-6, fd=0, Cmic_0=1e-6),
                   upper=c(Vmax=1e4, Km=1e4, CUE=0.999, kmic=1e5, ke=1e6, pe=1e6, fd=1, Cmic_0=25),niter=Niter)
