mem_init_a<-function(data, SUB, FACT, Mean, Niter, DNAci){
  ...
  if(SUB==TRUE){
    ...
    for(i in unique(dat$id)){
      obs_DNAc<-append(obs_DNAc, cost_DNAc(res[[i]]$bestpar, data=dat[dat$id==i, ])$residuals$obs)
      mod_DNAc<-append(mod_DNAc, cost_DNAc(res[[i]]$bestpar, data=dat[dat$id==i, ])$residuals$mod)
    }
