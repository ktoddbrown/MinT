...
The aim of this study is to quantify the effect of physical structure and $C_{S}$ quality on the coupled dynamic of $C_{S}$ and $C_{MB}$ quantified as two different $C_{MB}$ constituents (i.e. Cellular proteins and DNA). To do so, we designed an experiment in which indigenous soil microbial community was incubated with two different substrates (i.e. glucose and cellobiose) and with or without the presence of two types of physical barrier (i.e. glass beads and glass wool). We expect that the presence of physical barrier would decrease the rate of $C_{S}$ uptake by microbial community and as a result, decrease the growth of $C_{MB}$.
...
### Effect of $C_{MB}$ constituents on models fit
...
Structural complexity affected the estimate of all DB parameters except $Y_{S}$. $Y_{S}$ was the only parameter, which was possible to keep constant across structural complexity treatments without decreasing the correspondence between model simulations and observations. $V_{MAX}$ and $K_{M}$ parameters decreased in order BROTH > GLASS > WOOL. Nevertheless, the distribution of these parameters were highly correlated (data not shown). The estimated proteins content of Reserves decreased in the same order (Tab. 2) but the distribution of this parameter was not correlated with $V_{MAX}$ and $K_{M}$. The differencess between BROTH, GLASS and WOOL treatments in other parameters were small and did not follow any specific pattern.

## Discussion
### Models comparison
The growth of microbial biomass is regulated by $C_{S}$ concentration and the $C_{S}$ decay rate leading to $CO_{2}$ production (i.e. $R_{H}$) is regulated by the $C_{MB}$ size. This reciprocal relationship is the fundament of microbial-explicit biogeochemical models. In pure microbial cultures laboratory studies, $R_{H}$  and $C_{MB}$ are both simulated by a suite of available models with high accuracy (e.g. Bhunia2012). In laboratory studies conducted with soils, however, $R_{H}$ is simulated accurately whereas $C_{MB}$ simulation accuracy is always much lower (Wang et al., 2015; Huang et al., 2018, Salazar et al., 2018). The same was observed in our study and it means that the temporal dynamic of $C_{MB}$ is different than $R_{H}$ based model's expectation. Such an obvious inconsistency between the theory and masurements could be explained by the presence of the dormant microbial cells (Wang et al., 2015), changing microbial community structure (Buchkowski et al., 2018, Kaiser et al, 2017), diffusion limits (He et al., 2014, Gharasoo et al, 2012) or the methodological bias of $C_{MB}$ quantification. Even though the limitations of soil $C_{MB}$ quantification are known (e.g. Vance et al., 1987), they were rarely discussed in respect to calibration of microbial-explicit soil biogeochemical models. In line with Hanegraaf2000 and Hanegraaf2001, our results suggest that the composition of $C_{MB}$ is not constant (Fig. 5) and thus, $C_{MB}$ cannot be easily substituted by and measured as specific $C_{MB}$ constituent as it is normally done. When do so, the capability of microbial-explicit models to simulate $C_{MB}$ dynamic decreases to minimum (Tab. 1).
Two reasons for superiror capability of DB model to simulate the $C_{Proteins}$ and $C_{DNA}$ dynamic along with $R_{H}$ can be identified. First, two $C_{MB}$ pools (i.e. Reserves and Structures) provide a sufficient flexibility to explain the changing composition of $C_{MB}$ in respect to $C_{Proteins}$ and $C_{DNA}$ along the gradient of $C_{S}$ concentration (Hanegraafove). Second, R and S have different functions in microbial cell and thus, the function of particular $C_{MB}$ constituent in coupled $C_{S}$ and $C_{MB}$ dynamic depends on its abundance in R and/or S pool. DNA is present only in S pool (Hanegraaf2001) and therefore its concentration is related to $C_{S}$ consumption and maintnance respiration, which dominates the respiratory flux when $C_{S}$ concentration is low. This, together with the fact that DNA proportion to $C_{MB}$ doesn't change substantially in time (Fig. 5), could explain why Monod model was able to partly reproduce the dynamic of $C_{DNA}$ (Tab. 1). Cellular proteins, on the other hand, makes a dominant part of R pool (Tab. 2). R pool controls the growth respiration, which dominates the respiratory flux only during the brief period of time when $C_{S}$ concentration is high (Fig. 4). Its proportion to $C_{MB}$ changes in time dramatically (Fig. 5) and as a result, neither Monod nor MEND model were able to simulate $C_{Proteins}$ dynamic accurately (Tab. 1).

### Effect of experimental treatments
The effect of experimental treatments is usually complex, i.e. several model parameters can be affected by the treatment. For that reason, the calibration of microbial-explicit models should be ideally constrained by several variables. However, we showed how different $C_{MB}$ constituents used to constrain models calbibration affect the Monod and MEND model parameters estimates (Fig. 3). Their effect is larger than the effect of experimental treatment itself. When two similarly designed studies use different methods of $C_{MB}$ quantification, different conclussion could be reached. This becomes especially problemativ when the calibrated model is used to make projections to future.
