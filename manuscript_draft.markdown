...
Structural complexity affected the estimate fo all DB parameters except $Y_{S}$. $Y_{S}$ was the only parameter, which was possible to keep constant across structural complexity treatments without decreasing the correspondence between model simulations and observations. $V_{MAX}$ and $K_{M}$ parameters decreased in order BROTH > GLASS > WOOL. Nevertheless, the distribution of these parameters were highly correlated (data not shown). The estimated proteins content of Reserves decreased in the same order (Tab. 2) but the distribution of that estimate was not correlated with $V_{MAX}$ and $K_{M}$. The differencess between BROTH, GLASS and WOOL treatments in other parameters were small and did not follow any specific pattern. 
