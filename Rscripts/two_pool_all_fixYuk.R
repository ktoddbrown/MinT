two_pool_all_fixYuk<-function(data, FACT){
  ...
  deriv<-function(time, state, pars){
    ...
    with(as.list(c(state, pars)),{
      ...
      Cu=Vmax*C*S/(Km+C)
      #Respiration
      r=m0*S+f*R*(1-0.3565256)
      
      Protc=fpr*R+fps*S
      DNAc=fds*S
      
      dR<-Cu-f*R
      dS<-f*R*0.3565256 - m0*S - 0.004915818*S
      dC<--Cu + 0.004915818*S
  ...
  parnames<-c("Vmax", "Km", "m0", "f", "fpr", "fps", "fds")
  ...
  estim<-function(odeset){
    ...
    par_mcmc<-modMCMC(f=cost, p=c(Vmax=0.1, Km=3, m0=0.01, f=4, fpr=0.5, fps=0.5, fds=0.03),
                      lower=c(Vmax=1e-5, Km=1e-2, m0=1e-5, f=1e-3, fpr=0, fps=0, fds=0),
                      upper=c(Vmax=1, Km=20, m0=1, f=100, fpr=1, fps=1, fds=1), niter=10000)
  ...
  if(FACT==4){
    ...
    if(FACT==1){
      ...
      res$goodness$N<-rep(length(parnames)*2+3, times=3)
      ...
      if(FACT==2){
        ...
        res$goodness<-res$OvP %>% group_by(variable) %>% summarise(SSres=sum(((obs-value)^2), na.rm = T),
                                                                   ...
                                                                   ll=-sum(((obs-value)^2), na.rm = T)/2/(sd(obs, na.rm = T)^2))
        res$goodness$R2<-with(res$goodness, 1-SSres/SStot)
        res$goodness$N<-rep(length(parnames)*3+3, times=3)
        res$goodness$AIC<-with(res$goodness, 2*N-2*ll)
        ...
        res$goodness<-res$OvP %>% group_by(variable) %>% summarise(SSres=sum(((obs-value)^2), na.rm = T),
                                                                   ...
                                                                   ll=-sum(((obs-value)^2), na.rm = T)/2/(sd(obs, na.rm = T)^2))
        res$goodness$R2<-with(res$goodness, 1-SSres/SStot)
        res$goodness$N<-rep(length(parnames)*6+3, times=3)
        res$goodness$AIC<-with(res$goodness, 2*N-2*ll)
