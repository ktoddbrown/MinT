monodplus_all<-function(data, FACT){
  ...
  deriv<-function(time, state, pars){
    ...
    with(as.list(c(state, pars)),{
      ...
      return(list(c(dCmic, dC), r=r, DNAc=DNAc, Protc=Protc))
      
    })
  }
  #define names of parameters
  parnames<-c("Vmax", "Km", "CUE", "mr", "k", "fd", "fp")
  ...
  estim<-function(odeset){
    ...
    cost<-function(x){
      ...
      yhat_all<-as.data.frame(ode(y=c(Cmic=2.01*0.51/12.01/4/par[["fd"]], C=25), parms=par, deriv, times=sort(odeset$Time)))
      
      #select time and the measured variables 
      yhat<-select(yhat_all, c("time", "DNAc", "r", "Protc"))
      
      #reformat to long format data frame
      Yhat<-melt(yhat, id.vars = "time")
      
      #add the measured data to a data frame
      Yhat$obs<-c(odeset[order(odeset$Time), "DNAc"], odeset[order(odeset$Time), c("r")], odeset[order(odeset$Time), c("Protc")])
    ...
    par_mcmc<-modMCMC(f=cost, p=c(Vmax=0.1, Km=3, CUE=0.5, mr=0.01, k=1e-3, fd=0.5, fp=0.5), 
                      lower=c(Vmax=1e-3, Km=1e-3, CUE=0, mr=1e-5, k=1e-6, fd=0, fp=0),
                      upper=c(Vmax=10, Km=100, CUE=1, mr=10, k=10, fd=1, fp=1), niter=10000)
