two_pool_all_fixYukfVmaxKm<-function(data, FACT){
  ...
  estim<-function(odeset){
    ...
    par_mcmc<-modMCMC(f=cost, p=c(m0=0.01, fpr=0.5, fps=0.5, fds=0.03),
                      lower=c(m0=1e-5, fpr=0, fps=0, fds=0),
                      upper=c(m0=1, fpr=1, fps=1, fds=1), niter=10000)
  ...
  if(FACT==4){
    ...
    if(FACT==1){
      ...
      res$goodness$N<-rep(length(parnames)*2+6, times=3)
      ...
      if(FACT==2){
        ...
        res$goodness<-res$OvP %>% group_by(variable) %>% summarise(SSres=sum(((obs-value)^2), na.rm = T),
                                                                   ...
                                                                   ll=-sum(((obs-value)^2), na.rm = T)/2/(sd(obs, na.rm = T)^2))
        res$goodness$R2<-with(res$goodness, 1-SSres/SStot)
        res$goodness$N<-rep(length(parnames)*3+6, times=3)
        res$goodness$AIC<-with(res$goodness, 2*N-2*ll)
        ...
        res$goodness<-res$OvP %>% group_by(variable) %>% summarise(SSres=sum(((obs-value)^2), na.rm = T),
                                                                   ...
                                                                   ll=-sum(((obs-value)^2), na.rm = T)/2/(sd(obs, na.rm = T)^2))
        res$goodness$R2<-with(res$goodness, 1-SSres/SStot)
        res$goodness$N<-rep(length(parnames)*6+6, times=3)
        res$goodness$AIC<-with(res$goodness, 2*N-2*ll)
