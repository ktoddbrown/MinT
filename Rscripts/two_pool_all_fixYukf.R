two_pool_all_fixYukf<-function(data, FACT){
  ...
  parnames<-c("Vmax", "Km", "m0", "fpr", "fps", "fds")
  ...
  estim<-function(odeset){
    ...
    par_mcmc<-modMCMC(f=cost, p=c(Vmax=0.1, Km=3, m0=0.01, fpr=0.5, fps=0.5, fds=0.03),
                      lower=c(Vmax=1e-5, Km=1e-2, m0=1e-5, fpr=0, fps=0, fds=0),
                      upper=c(Vmax=1, Km=20, m0=1, fpr=1, fps=1, fds=1), niter=10000)
  ...
  if(FACT==4){
    ...
    if(FACT==1){
      ...
      res$goodness$N<-rep(length(parnames)*2+4, times=3)
      ...
      if(FACT==2){
        ...
        res$goodness<-res$OvP %>% group_by(variable) %>% summarise(SSres=sum(((obs-value)^2), na.rm = T),
                                                                   ...
                                                                   ll=-sum(((obs-value)^2), na.rm = T)/2/(sd(obs, na.rm = T)^2))
        res$goodness$R2<-with(res$goodness, 1-SSres/SStot)
        res$goodness$N<-rep(length(parnames)*3+4, times=3)
        res$goodness$AIC<-with(res$goodness, 2*N-2*ll)
        ...
        res$goodness<-res$OvP %>% group_by(variable) %>% summarise(SSres=sum(((obs-value)^2), na.rm = T),
                                                                   ...
                                                                   ll=-sum(((obs-value)^2), na.rm = T)/2/(sd(obs, na.rm = T)^2))
        res$goodness$R2<-with(res$goodness, 1-SSres/SStot)
        res$goodness$N<-rep(length(parnames)*6+4, times=3)
        res$goodness$AIC<-with(res$goodness, 2*N-2*ll)
