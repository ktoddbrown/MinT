monodplus_prot_fix<-function(data, FACT){
  ...
  parnames<-c("Vmax", "Km", "CUE", "mr", "k","fp")
  ...
  estim<-function(odeset){
    ...
    par_mcmc<-modMCMC(f=cost, p=c(Vmax=0.1, Km=3, CUE=0.5, mr=0.01, k=1e-3, fp=0.5), 
                      lower=c(Vmax=1e-3, Km=1e-3, CUE=0, mr=1e-5, k=1e-6, fp=0),
                      upper=c(Vmax=10, Km=100, CUE=1, mr=10, k=10, fp=1), niter=10000)
  ...
  if(FACT==4){
    ...
    if(FACT==1){
      ...
      res$goodness$N<-rep(length(parnames)*2+1, times=2)
      ...
      if(FACT==2){
        ...
        res$goodness<-res$OvP %>% group_by(variable) %>% summarise(SSres=sum(((obs-value)^2), na.rm = T), 
                                                                   ...
                                                                   ll=-sum(((obs-value)^2), na.rm = T)/2/(sd(obs, na.rm = T)^2))
        res$goodness$R2<-with(res$goodness, 1-SSres/SStot)
        res$goodness$N<-rep(length(parnames)*3+1, times=2)
        res$goodness$AIC<-with(res$goodness, 2*N-2*ll)
        ...
        res$goodness<-res$OvP %>% group_by(variable) %>% summarise(SSres=sum(((obs-value)^2), na.rm = T), 
                                                                   ...
                                                                   ll=-sum(((obs-value)^2), na.rm = T)/2/(sd(obs, na.rm = T)^2))
        res$goodness$R2<-with(res$goodness, 1-SSres/SStot)
        res$goodness$N<-rep(length(parnames)*6+1, times=2)
        res$goodness$AIC<-with(res$goodness, 2*N-2*ll)
