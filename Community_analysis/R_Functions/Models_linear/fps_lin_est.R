fps_lin_est<-function(odeset, par_const){
  #Cost function
  cost<-function(x){
    p<-x
    #p<-out_const$pars
    #names(p)<-c("Ecat", "Mr","f", "chi", "Yue", "j", "k", "fpr", "fps", "fds", "RSinit", "I", "B")
    names(p)<-c("Ecat", "Mr","f", "chi", "Yue", "j", "k", "fpr", "fds", "RSinit", "I", "B")
  ...
  par_c<-par_const[-which(names(par_const)=="fps")]
  par_t<-par_const[["fps"]]
