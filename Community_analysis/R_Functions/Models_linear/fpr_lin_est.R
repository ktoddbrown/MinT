fpr_lin_est<-function(odeset, par_const){
  #Cost function
  cost<-function(x){
    p<-x
    #p<-out_const$pars
    #names(p)<-c("Ecat", "Mr","f", "chi", "Yue", "j", "k", "fpr", "fps", "fds", "RSinit", "I", "B")
    names(p)<-c("Ecat", "Mr","f", "chi", "Yue", "j", "k", "fps", "fds", "RSinit", "I", "B")
  ...
  par_c<-par_const[-which(names(par_const)=="fpr")]
  par_t<-par_const[["fpr"]]
  par_mcmc<-modMCMC(f=cost, p=c(par_c, I=par_t, B=1),
                    lower=c(par_c*0.5, I=par_t*0.5, B=-100),
                    upper=c(Ecat=10, Mr=1e-1, f=3e-1, chi=1, Yue=0.9, j=1, 
                            k=1e-1, fps=1, fds=1, RSinit=50, 
