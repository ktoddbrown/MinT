DB_constant<-function(data){
  ...
  DB<-function(time, state, pars){
    ...
    with(as.list(c(state, pars)),{
      ...
      Protinc=fpr*R+fps*S
      DNAc=fds*S
      Protoutc=Enz + pmax(0, -(1 - j)*(an/Mr))*fps
  ...
  estim<-function(odeset){
    ...
    cost<-function(x){
      ...
      #Initial conditions
      Sinit<-odeset$DNA.initc[1]/p[["fds"]]
      ...
      #Filter the output
      yhat <- yhat_all[, c("Protinc", "DNAc", "Protoutc", "r", "time")]
    ...
    par_mcmc<-modMCMC(f=cost, p=c(Ecat=0.01, Mr=3e-3, f=3e-4, chi=0.5, Yue=0.6, j=0.5, 
                      ...
                      upper=c(Ecat=10, Mr=3e-1, f=3e-1, chi=1, Yue=1, j=1, 
