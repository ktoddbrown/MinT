...
##Tables
```{r Tab. 1, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, results='asis'}
...
kable(tab1, align='c', digits=c(0, 1, 0, 2, 1), caption = "_**Table 1:** Correspondence between measured resiration rate and respiration rate modeled by five different models (First order decay, Monod growth, two different Microbial Enzyme Models (MEM and MEND) and Metabolic Microbial Enzyme Model (MMEM)). Log Likelihood, coefficient of determination ($R^{2}$), Akaike Information Criterion (AIC) and the number of estimated parameters are reported. All models are calibrated against measured respiration rate only._")
```

##Figures
```{r Scheme 1, eval=FALSE, fig.height=6, fig.width=6, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}

ggplot(data.frame(a=0, b=0), aes(a,b))+geom_blank()+
  theme(axis.text.x = element_blank(), axis.title.x = element_blank(),
        axis.text.y = element_blank(), axis.title.y = element_blank(),
        panel.background = element_blank(),
        axis.line.x = element_blank(), axis.line.y = element_blank(),
        axis.ticks.x = element_blank(), axis.ticks.y = element_blank())+
  geom_rect(aes(xmin=0.8, xmax=1.2, ymin=0.65, ymax=0.95), fill="white", colour="black", lwd=1.2)+
  geom_text(aes(x=1, y=0.8, label="DOC"), size=14)+ylim(0.5,1.5)+xlim(0.5,1.5)+
  geom_text(aes(x=1, y=1.3, label="CO[2]"), size=14, parse = T)+
  geom_text(aes(x=1, y=1.5, label=c("First order decay")), size=11, parse = F, fontface="italic")+
  geom_segment(aes(x=1, y=0.95, xend=1, yend=1.25), lwd=1.2, arrow = arrow(length = unit(0.5, "cm")))
...
ggplot(data.frame(a=0, b=0), aes(a,b))+geom_blank()+
  theme(axis.text.x = element_blank(), axis.title.x = element_blank(),
        ...
        axis.line.x = element_blank(), axis.line.y = element_blank(),
        axis.ticks.x = element_blank(), axis.ticks.y = element_blank())+
  geom_rect(aes(xmin=1.1, xmax=1.5, ymin=0.65, ymax=0.95), fill="white", colour="black", lwd=1.2)+
  geom_text(aes(x=1.3, y=0.8, label="DOC"), size=14)+ylim(0.5,1.5)+xlim(0.5,1.5)+
  geom_text(aes(x=1.3, y=1.3, label="CO[2]"), size=14, parse = T)+
  geom_text(aes(x=1, y=1.5, label=c("Monod growth")), size=11, parse = F, fontface="italic")+
  geom_segment(aes(x=1.3, y=0.95, xend=1.3, yend=1.25), lwd=1.2, arrow = arrow(length = unit(0.5, "cm")))+
  geom_rect(aes(xmin=0.6, xmax=0.8, ymin=0.65, ymax=0.95), fill="white", colour="black", lwd=1.2)
...
```{r Fig. 1, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.height=16, fig.width=9, fig.cap="_**Fig. 1:** Distribution of microbial biomass carbon data as directly measured by the DNA or cellular protein concentration and calculated by different conversion factors published in the literature [@Baart2008; @Beck2018; @VanDuuren2013; @Marstorp2000, @Christensen1993; Christensen1993; @Yokoyama2017; @VanPutten1995; @Henriksen1996; @Hanegraaf2001]. Middle line corresonds to the median, lower and upper hinges to first and third quartile respectively, lower and upper whisker extends from lower and upper hinge to 1.5 time the interquartile range. Note that the x axis of both plots have different scales._"}
