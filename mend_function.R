mend_function<-function(data, FACT, Vars, ColM, Mean, Cmic, Niter){
  ...
  if(FACT==1){
    ...
    deriv<-function(time, state, pars){
      ...
      with(as.list(c(state, pars)),{
        ...
        return(list(c(dCmic, dC, dE), r=F4+F5))
    ...
    estim<-function(data){
      ...
      res<-modMCMC(f=cost_function, p=c(Vmax=0.1, Km=3, CUE=0.8, mr=0.01, ke=0.01, pe=0.5),
                   lower=c(Vmax=1e-4, Km=1e-4, CUE=1e-2, mr=1e-5, ke=1e-6, pe=0),
                   upper=c(Vmax=1e4, Km=1e4, CUE=0.999, mr=1e5, ke=1e6, pe=1),niter=Niter)
    ...
    parameters<-data.frame(Vmax=vector("numeric", length = length(unique(dat$id))), 
                           ...
                           mrc=vector("numeric", length = length(unique(dat$id))),
    ...
    #lower quartile
    for(i in unique(dat$id)){
      for(n in 1:((ncol(parameters)-1)/3)*2){
