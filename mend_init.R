mend_init<-function(data, SUB, FACT, Mean, Niter, DNAci){
  ...
  if(SUB==TRUE){
    ...
    mend<-function(X, pars, t){
      ...
      deriv<-function(time, state, pars){
        ...
        with(as.list(c(state, pars)),{
          ...
          return(list(c(dCmic, dC, dE), r=F4+F5, DNAc=DNAc, Protc=Protc))
    ...
    estim<-function(data){
      ...
      res<-modMCMC(f=cost_function, p=c(Vmax=0.1, Km=3, CUE=0.8, mr=0.01, ke=0.01, pe=0.5, fd=0.05, fp=0.5, Cmic_0=0.12),
                   lower=c(Vmax=1e-4, Km=1e-4, CUE=0, mr=1e-5, ke=1e-6, pe=0, fd=0, fp=0, Cmic_0=1e-6),
                   upper=c(Vmax=1e4, Km=1e4, CUE=1, mr=1e5, ke=1e6, pe=1, fd=1, fp=1, Cmic_0=25),niter=Niter)
