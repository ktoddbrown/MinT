andrew_init_b<-function(data, SUB, FACT, Mean, DNAci, Niter){
  ...
  if(SUB==TRUE){
    ...
    mmem<-function(X, pars, t){
      ...
      deriv<-function(time, state, pars){
        ...
        with(as.list(c(state, pars)),{
          ...
          return(list(c(dCmic, dC, dE), r=r, Protc=Protc))
    ...
    estim<-function(data){
      ...
      res<-modMCMC(f=cost_function, p=c(umax=0.1, Km=3, Ki=0.08, Ymic=0.8, Ye=0.8, alfa=2, m=0.001, kmic=0.01, ke=0.01, fp=0.5, Cmic_0=0.12),
                   lower=c(umax=0.001, Km=0.03, Ki=0.0008, Ymic=0.008, Ye=0.008, alfa=0.02, m=0.00001, kmic=0.0001, ke=0.0001,  fp=0, Cmic_0=1e-6),
                   upper=c(umax=100, Km=50, Ki=8, Ymic=1, Ye=1, alfa=20,  m=10, kmic=10, ke=10,  fp=1, Cmic_0=25),niter=Niter)
    ...
    #   #mtrue<-rbind(Obs_dat, data.frame(time=0, r=NA, E=NA, DNAc=DNAci, Protc=NA))  
    #   
    #   mtrue<-select(Obs_dat, c("time", "DNAc"))
    #   
    #   out<-as.data.frame(ode(y=c(Cmic=pars[["Cmic_0"]], C=cinit, E=0), parms=pars, times=seq(0,130), func=deriv))
    #   cost<-modCost(model = out, obs = mtrue)
    #   
    ...
    cost_Protc<-function(pars, data){
      ...
      mtrue<-select(Obs_dat, c("time", "Protc"))
