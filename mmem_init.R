mmem_init<-function(data, SUB, FACT, Mean, DNAci, Niter){
  ...
  if(SUB==TRUE){
    ...
    mmem<-function(X, pars, t){
      ...
      deriv<-function(time, state, pars){
        ...
        with(as.list(c(state, pars)),{
          ...
          return(list(c(dCmic, dC, dE), r=Cu*psi.r, DNAc=DNAc, Protc=Protc))
    ...
    estim<-function(data){
      ...
      res<-modMCMC(f=cost_function, p=c(Vmax=0.1, Km=3, kmic=0.01, ke=0.01, x=0.5, Yatp=1, w=0.5, PO=1.5, fd=0.05, fp=0.5, Cmic_0=0.12),
                   lower=c(Vmax=1e-4, Km=1e-4, kmic=1e-5, ke=1e-6, x=0, Yatp=0.0001, w=0, PO=0.1, fd=0, fp=0, Cmic_0=1e-6),
                   upper=c(Vmax=1e4, Km=1e4, kmic=1e5, ke=1e6, x=1, Yatp=30,w=1, PO=3, fd=1, fp=1, Cmic_0=25),niter=Niter)
